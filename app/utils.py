...
class SimpleFallbackModel(torch.nn.Module):
    def __init__(self, num_classes=2, img_size=IMG_SIZE):
        super().__init__()
        self.linear = torch.nn.Linear(3 * img_size * img_size, num_classes)
...
def load_model_for_serving(model_path, device='cpu'):
    ...
    Falls back to a lightweight stub model if the artifact is missing or invalid.
    """
    if not os.path.exists(model_path):
        return SimpleFallbackModel(num_classes=len(CLASS_NAMES)).to(device).eval()
    ...
    try:
        ...
        fallback = SimpleFallbackModel(num_classes=len(CLASS_NAMES)).to(device).eval()
        return fallback
